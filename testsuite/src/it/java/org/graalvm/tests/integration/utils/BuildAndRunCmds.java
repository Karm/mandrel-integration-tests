...
public enum BuildAndRunCmds {
    ...
    DEBUG_QUARKUS_FULL_MICROPROFILE(new String[][] {
            new String[] { "mvn", "package", "-Pnative", "-Dquarkus.native.debug.enabled=true",
                    "-Dquarkus.version=" + QUARKUS_VERSION.getVersionString(),
                    "-Dquarkus.native.additional-build-args=" +
                            "-H:Log=registerResource:," +
                            "--trace-object-instantiation=java.util.Random," +
                            "--initialize-at-run-time=io.vertx.ext.auth.impl.jose.JWT"
    ...
    QUARKUS_FULL_MICROPROFILE_PERF(new String[][] {
            ...
            new String[] { "perf", "stat", "java", "-Xlog:gc", "-XX:+UseSerialGC", "-Xmx" + MX_HEAP_MB + "m", "-jar",
    ...
    QUARKUS_JSON_PERF_PARSEONCE(new String[][] {
            ...
            new String[] { "mvn", "package", "-Pnative", "-Dquarkus.version=" + QUARKUS_VERSION.getVersionString(),
                    "-Dquarkus.native.additional-build-args=" +
                            ...
                            GRAALVM_BUILD_OUTPUT_JSON_FILE + "+ParseOnce",
                    "-Dcustom.final.name=quarkus-json_+ParseOnce" },
            new String[] { "mvn", "package", "-Dcustom.final.name=quarkus-json",
                    "-Dquarkus.version=" + QUARKUS_VERSION.getVersionString() },
            //-XX:+UseShenandoahGC
            //-XX:+UseSerialGC
            //-XX:+UseG1GC
            new String[] { "perf", "stat", "--delay", "2000", "java", "-Xlog:gc", "-XX:+UseSerialGC", "-Xmx" + MX_HEAP_MB + "m",
    ...
    QUARKUS_BUILDER_IMAGE_ENCODING(new String[][] {
            new String[] { "mvn", "package", "-Pnative", "-Dquarkus.native.container-build=true",
                    ...
                    "-Dquarkus.version=" + QUARKUS_VERSION.getVersionString() },
            new String[] { CONTAINER_RUNTIME, "build", "-f", "src/main/docker/Dockerfile.native", "-t",
                    "my-quarkus-mandrel-app", "." },
    ...
    DEBUG_QUARKUS_BUILDER_IMAGE_VERTX(new String[][] {
            ...
            new String[] { CONTAINER_RUNTIME, "build", "--network=host", "-f", "src/main/docker/Dockerfile.native", "-t",
                    "my-quarkus-mandrel-app", "." },
            ...
            new String[] { CONTAINER_RUNTIME, "run", "--network=host", "--cap-add=SYS_PTRACE",
                    "--security-opt=seccomp=unconfined",
                    "-i", "-d", "--rm", "--name", ContainerNames.DEBUG_QUARKUS_BUILDER_IMAGE_VERTX.name,
                    "my-quarkus-mandrel-app" }
    }),
    ...
    TIMEZONES(new String[][] {
            new String[] { "mvn", "package" },
            new String[] { "native-image", "-J-Duser.country=CA", "-J-Duser.language=fr", "-jar", "target/timezones.jar",
                    "target/timezones" },
            new String[] { IS_THIS_WINDOWS ? "target\\timezones.exe" : "./target/timezones" }
    }),
    CALENDARS(new String[][] {
            new String[] { "mvn", "package" },
            new String[] { "native-image", "--link-at-build-time=calendar.Main", "-jar", "target/calendars.jar",
                    "target/calendars" },
            new String[] { IS_THIS_WINDOWS ? "target\\calendars.exe" : "./target/calendars" }
    }),
    RECORDANNOTATIONS(new String[][] {
            new String[] { "mvn", "package" },
            new String[] { "native-image", "--no-fallback", "-jar", "target/recordannotations.jar",
                    "target/recordannotations" },
            new String[] { IS_THIS_WINDOWS ? "target\\recordannotations.exe" : "./target/recordannotations" }
    }),
    VERSIONS(new String[][] {
            new String[] { "mvn", "package" },
            new String[] { "native-image", "--features=org.graalvm.home.HomeFinderFeature", "-jar", "target/version.jar",
                    "target/version" },
            new String[] { IS_THIS_WINDOWS ? "target\\version.exe" : "./target/version" }
    }),
    IMAGEIO(new String[][] {
            new String[] { "mvn", "clean", "package" },
            new String[] { "java", "-Djava.awt.headless=true",
                    "-agentlib:native-image-agent=config-output-dir=src/main/resources/META-INF/native-image", "-jar",
                    "target/imageio.jar" },
            new String[] { "jar", "uf", "target/imageio.jar", "-C", "src/main/resources/", "META-INF" },
            new String[] { "native-image", "-J-Djava.awt.headless=true", "--no-fallback", "-jar", "target/imageio.jar",
                    "target/imageio" },
            new String[] { IS_THIS_WINDOWS ? "target\\imageio.exe" : "./target/imageio", "-Djava.home=.",
    ...
    IMAGEIO_BUILDER_IMAGE(new String[][] {
            ...
            new String[] { CONTAINER_RUNTIME, "run", IS_THIS_WINDOWS ? "" : "-u", IS_THIS_WINDOWS ? "" : getUnixUIDGID(),
                    "-t", "--entrypoint", "java", "-v",
                    BASE_DIR + File.separator + "apps" + File.separator + "imageio:/project:z",
            ...
            new String[] { CONTAINER_RUNTIME, "run", IS_THIS_WINDOWS ? "" : "-u", IS_THIS_WINDOWS ? "" : getUnixUIDGID(),
                    ...
                    ContainerNames.IMAGEIO_BUILDER_IMAGE.name, "/work/target/imageio", "-Djava.home=.",
                    "-Djava.awt.headless=true" }
    }),
    DEBUG_SYMBOLS_SMOKE(new String[][] {
            ...
            IS_THIS_WINDOWS
                    ? new String[] { "powershell", "-c",
                            "\"Expand-Archive -Path test_data.txt.zip -DestinationPath target -Force\"" }
    ...
    JFR_PERFORMANCE(new String[][] {
            ...
            new String[] { "./target/jfr-perf-runner",
                    "-XX:+FlightRecorder",
                    "-XX:StartFlightRecording=settings=" + BASE_DIR + File.separator + "apps" + File.separator
                            + "jfr-native-image-performance/jfr-perf.jfc,filename=logs/flight-native.jfr",
                    "-XX:FlightRecorderLogging=jfr" },
            new String[] { CONTAINER_RUNTIME, "run", "--name", ContainerNames.HYPERFOIL.name, "--rm", "--network=host",
    ...
    JFR_PERFORMANCE_BUILDER_IMAGE(new String[][] {
            new String[] { "mvn", "clean", "package", "-Pnative", "-Dquarkus.native.container-build=true",
                    "-Dquarkus.native.container-runtime=" + CONTAINER_RUNTIME,
                    "-Dquarkus.native.builder-image=" + BUILDER_IMAGE,
                    "-Dquarkus.version=" + QUARKUS_VERSION.getVersionString(), "-Dquarkus.native.monitoring=jfr",
                    "-Dquarkus.native.additional-build-args=-H:+SignalHandlerBasedExecutionSampler",
                    "-DfinalName=jfr-perf" },
    ...
    JFR_SMOKE(new String[][] {
            ...
            IS_THIS_WINDOWS
                    ? new String[] { "powershell", "-c",
                            "\"Expand-Archive -Path test_data.txt.zip -DestinationPath target -Force\"" }
                    : new String[] { "unzip", "test_data.txt.zip", "-d", "target" },
            new String[] { "native-image", JFR_MONITORING_SWITCH_TOKEN, "-jar", "target/debug-symbols-smoke.jar",
    ...
    JFR_SMOKE_BUILDER_IMAGE(new String[][] {
            ...
            new String[] {
                    ...
                    BUILDER_IMAGE, JFR_MONITORING_SWITCH_TOKEN, "-jar", "target/debug-symbols-smoke.jar",
                    "target/debug-symbols-smoke" },
            new String[] {
                    CONTAINER_RUNTIME, "run", "-u", IS_THIS_WINDOWS ? "" : getUnixUIDGID(),
                    "-i",
                    "--entrypoint", "java", "-v",
                    BASE_DIR + File.separator + "apps" + File.separator + "debug-symbols-smoke:/project:z",
